          ...
          <article class="tile is-child box">
            ...
            <tabs :only-fade="false">
              <tab-pane label="Late Night" selected>
                ...
                <data-table v-if="searchQueryLN && lateNight !== null"
                  :data="filteredLN"
                  :columns-data="gridColumns"
                  :columns="columnNames"
                  :filter-key="searchQueryLN"
                  mode="permitsdatabase"
                ></data-table>
              ...
              <tab-pane label="Overnight">
                ...
                <data-table v-if="searchQueryON && overNight !== null"
                  :data="filteredON"
                  :columns-data="gridColumns"
                  :columns="columnNames"
              ...
              <tab-pane label="Early Morning">
                ...
                <data-table v-if="searchQueryEM && earlyMorning !== null"
                  :data="filteredEM"
                  :columns-data="gridColumns"
                  :columns="columnNames"
...
export default {
  created () {
    this.axios.get('permits/all/').then(
      response => {
  ...
  methods: {
    ...
    downloadCSV: function () {
      window.location.href = 'https://e.updkalay.com/api/v1/permits/csvall'
    }
  },
  data () {
    return {
      ...
      searchQueryON: '',
      searchQueryEM: '',
      columnNames: ['Student Number', 'Location', 'Reason', 'Date/Time', 'Status'],
      gridColumns: ['upid', 'location', 'reason', 'data_one', 'approved_dm'],
      startRowLN: 0,
      startRowON: 0,
      startRowEM: 0,
      rowsPerPage: 100000
